import { getInterview, getInterviewerForDay } from "./selectors";
...
test.only("getInterviewerForDay returns an array", () => {
  const result = getInterviewerForDay(state, "Monday");
  expect(Array.isArray(result)).toBe(true);
});

test.only("getInterviewerForDay returns an array with a length matching the number of interviewers for that day", () => {
  const result = getInterviewerForDay(state, "Monday");
  expect(result.length).toEqual(1);
});

test.only("getInterviewerForDay returns an array containing the correct interviewer objects", () => {
...
test.only("getInterviewerForDay returns an empty array when the interviewer data is empty", () => {
  const result = getInterviewerForDay({ days: [] }, "Monday");
  expect(result.length).toEqual(0);
});

test.only("getInterviewerForDay returns an empty array when the interviewer is not found", () => {
